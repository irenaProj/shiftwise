...
test.describe('Employee Management', () => {
  test('dashboard shows employee list', async ({ authenticatedPage: page }) => {
    ...
    await expect(page.getByRole('table').getByText('Fran Tastic')).toBeVisible()
  })

  test('shows stats strip with correct counts', async ({ authenticatedPage: page }) => {
  ...
  test('manager can open Add member modal', async ({ authenticatedPage: page }) => {
